...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.error('[entrepreneur-validate-idea] No authorization header');
      return new Response(
    ...
    if (authError || !user) {
      console.error('[entrepreneur-validate-idea] Auth error:', authError);
      return new Response(
        JSON.stringify({ error: 'Unauthorized' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    console.log(`[entrepreneur-validate-idea] Processing request for user ${user.id}`);
