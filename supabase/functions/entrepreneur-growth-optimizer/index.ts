...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.error('[entrepreneur-growth-optimizer] No authorization header');
      return new Response(
    ...
    if (authError || !user) {
      console.error('[entrepreneur-growth-optimizer] Auth error:', authError);
      return new Response(
        JSON.stringify({ error: 'Unauthorized' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    console.log(`[entrepreneur-growth-optimizer] Processing ${action} request for user ${user.id}`);
