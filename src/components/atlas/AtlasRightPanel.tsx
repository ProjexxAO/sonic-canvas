...
import { Activity, Database, Search, Sparkles, Brain, Shield, User, Camera, Phone, Bell, Upload, Image, GripVertical } from 'lucide-react';
...
export function AtlasRightPanel({
  ...
  if (hubType === 'personal') {
    ...
    // Desktop/Tablet layout - with horizontal resize capability
    return (
      <PanelGroup direction="horizontal" className="flex-1 min-w-0 h-full">
        ...
        <PanelResizeHandle className="w-2 flex items-center justify-center group cursor-col-resize hover:bg-muted/50 transition-colors mx-1">
          <div className="flex flex-col items-center gap-0.5">
            <GripVertical size={14} className="text-muted-foreground group-hover:text-primary transition-colors" />
          </div>
        </PanelResizeHandle>

        {/* Quick Access Sidebar Panel */}
        <Panel defaultSize={25} minSize={10} maxSize={40}>
          <div className="h-full bg-card/50 border border-border rounded-lg p-3 overflow-hidden flex flex-col">
            <h3 className="text-[10px] font-mono text-muted-foreground uppercase mb-3">Quick Access</h3>
            <ScrollArea className="flex-1">
              <div className="space-y-2">
                <HubQuickAccess className="flex-col items-stretch" />
              </div>
            </ScrollArea>
          </div>
        </Panel>
